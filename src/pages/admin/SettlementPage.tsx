...
const STATUS_EMPHASIS: TransactionStatus[] = [
  'completed',
  'pending',
  'failed',
]
...
export default function SettlementPage() {
  const { transactions, updateTransaction, appendLog } = useAdminData()
  ...
  const fidelityRows = useMemo(
    () =>
      transactions.filter(
        (t) => usesFidelityPayRail(t.channel) && t.status !== 'reconciled',
      ),
    [transactions],
  )
    ...
    <div className="space-y-8">
      ...
      <section aria-label="Settlement status breakdown">
        <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.14em] text-zinc-400">
          Status mix (Fidelity routes only)
        </p>
      ...
      >
        <div className="border-b border-zinc-100 bg-linear-to-r from-white to-zinc-50/90 px-5 py-4">
          <p className="text-sm font-medium text-zinc-700">
            <span className="tabular-nums font-bold text-zinc-950">{total}</span> Fidelity
            settlement row{total === 1 ? '' : 's'} · newest first
          </p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[880px] border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-zinc-100">
              {paginated.map((t) => (
                <tr key={t.id} className="transition hover:bg-sky-50/40">
